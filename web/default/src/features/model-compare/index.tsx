...
> = {
  ...
  'deepseek-v4-flash':           { name: 'DeepSeek V4 Flash',      company: 'DeepSeek',    region: 'cn', tier: 'low',    pricePerKToken: 0.002  },
}
...
const QUICK_PROMPTS: Array<{ labelKey: string; text: string }> = [
  { labelKey: 'Default Easy Question',   text: '我需要洗车。洗车场在100米远的地方。我应该开车去还是走着去？' },
  { labelKey: 'Default Medium Question', text: '农夫带着狼、羊和白菜过河，小船一次只能载农夫和一样东西。狼会吃羊，羊会吃白菜。怎么安全过河？' },
]

const MIN_MODELS = 3
const MAX_MODELS = 3
const DEFAULT_MODEL_IDS = ['doubao-seed-2-0-lite-260428', 'gpt-5.4-mini', 'deepseek-v4-flash']
...
export function ModelComparePanel() {
  ...
  const applyQuickPrompt = (text: string) => setPromptInput(text)
  ...
  const sendPrompt = async (text?: string) => {
    ...
    storeSet((prev) => [...prev, { id: roundId, prompt, results: loadingResults }])
              ...
              <div className='flex h-full items-center justify-center rounded-xl border border-dashed p-6 text-center'>
                <div className='max-w-xl space-y-3'>
                  ...
                  <p className='text-muted-foreground text-sm'>
                    {t('Select exactly 3 models, then ask a question to start comparison.')}
                  </p>
                </div>
              </div>
            ) : (
        ...
        <DialogContent className='sm:max-w-4xl'>
          ...
          <DialogFooter>
            <div className='flex w-full items-center justify-between'>
              <p className='text-xs text-muted-foreground'>
                {t('Selected {{count}} models (required: {{required}})', { count: selectedModelIds.length, required: MIN_MODELS })}
              </p>
              <Button
                disabled={selectedModelIds.length !== MIN_MODELS}
